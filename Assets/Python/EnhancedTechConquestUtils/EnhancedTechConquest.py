...
def loadConfigurationData():
	global g_bCheckPrereq, g_iBasePercent, g_iPopPercent
	import BugPath, ConfigParser
	BugPath.getModDir()
	path = BugPath.getModDir() + "\Assets\TechConquest.ini"
	Config = ConfigParser.ConfigParser()
	Config.read(path)

	if Config:
		g_bCheckPrereq = Config.get("Enhanced Tech Conquest", "Check Prereq") not in ("False", "false", "0")

		g_iBasePercent = Config.get("Enhanced Tech Conquest", "Base Percent")
		if g_iBasePercent.isdigit():
			g_iBasePercent = int(g_iBasePercent)
		else: g_iBasePercent = 0
...
class EnhancedTechConquest:
	...
	def onCityAcquired(self, argsList):
		...
		if CyPlayerN.isBarbarian(): return
		...
		if aList:
			...
			if iOwnerNew == GAME.getActivePlayer():
				...
				CyInterface().addMessage(iOwnerNew, True, 20, szTxt, "", 0, GC.getCivilizationInfo(CyPlayerO.getCivilizationType()).getButton(), ColorTypes(12), CyCity.getX(), CyCity.getY(), True, True) 
