		...
		for group1 in grouping1.itergroups():
			...
			for group2 in grouping2.itergroups():
				...
				for unit in units2:
					...
					screen.setMinimapColor(MinimapModeTypes.MINIMAPMODE_MILITARY, gc.getMap().getViewportXFromMapX(loopUnit.getX()), gc.getMap().getViewportYFromMapY(loopUnit.getY()), iColor, 0.6)
					if (bUnitSelected or bGroup2Selected) and iPlayer in self.selectedLeaders:
						...
						if (player.getTeam().isAtWar(gc.getPlayer(self.iActivePlayer).getTeam())):
							...
							iColor = eWhite
						screen.minimapFlashPlot(gc.getMap().getViewportXFromMapX(loopUnit.getX()), gc.getMap().getViewportYFromMapY(loopUnit.getY()), iColor, -1)
...
#			screen.addPanel(szPanel_ID, u"", "", False, False, iPanel_X, iPanel_Y, iPanel_W, iPanel_H, PanelStyles.PANEL_STYLE_MAIN)
	...
	def minimapClicked(self):
		CyMap().closeAdvisor(self.W_SCREEN, self.X_MAP + self.MAP_MARGIN, self.X_MAP + self.W_MAP - self.MAP_MARGIN, self.Y_MAP + self.MAP_MARGIN, self.Y_MAP + self.H_MAP - self.MAP_MARGIN)
		self.hideScreen()
