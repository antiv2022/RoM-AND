...
#DancinhHoskuld - max help characters per line - start
		if (screen.getXResolution() < 1100):
			self.MAX_HELP_CHARS_PER_LINE = int(screen.getXResolution() / 9) - 20
		if (screen.getXResolution() < 1300):
			self.MAX_HELP_CHARS_PER_LINE = int(screen.getXResolution() / 9) - 40
		else:
	...
	def drawHelpText(self, iCivicOption):
		...
		if (len(szStrategy) > 2):
			...
			for item in szFormattedStrategyList:	
				item = BugUtil.colorText(item, "COLOR_LIGHT_GREY")
...
#DancinhHoskuld - max help characters per line - end
	...
	def getNumberCivicChoices(self, iCivicOption):
		k = 0
	...
	def getRealHeadingHeight(self, iCivicOption):
		...
		if (screen.getXResolution() > 1600):
			return 5 + int(lines * 27.5)
		if (screen.getXResolution() > 1400):
			return 20 + int(lines * 27.5)
		return 60 + int(lines * 27.5)
	...
	def Revolution(self, inputClass):
		...
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED) :
			# f1rpo (advc.001d): Clause added to prevent revolution when viewing another civ's civics through the Debug menu
